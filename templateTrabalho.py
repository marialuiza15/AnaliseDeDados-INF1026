...
ARQUIVO_SUS   = 'DO_RJ_2024.csv'
ARQUIVO_INMET = 'inmet_consolidado.csv'
ARQUIVO_IBGE  = 'municipios_ibge.csv'

dfSUS   = pd.read_csv(ARQUIVO_SUS,   dtype=str)
dfINMET = pd.read_csv(ARQUIVO_INMET)
dfIBGE  = pd.read_csv(ARQUIVO_IBGE)

print('\n==============================================')
print('Revisão P2 — Vigilância Epidemiológica / SIM · INMET · IBGE')
print('==============================================\n')

print('Amostra inicial - dfSUS:')
print(dfSUS[['DTOBITO', 'IDADE', 'SEXO', 'RACACOR', 'CAUSABAS',
             'CODMUNRES', 'LOCOCOR', 'COVID_CLAS']].head(3))
print('\nAmostra inicial - dfINMET:')
print(dfINMET.head(3))
print('\nAmostra inicial - dfIBGE:')
print(dfIBGE.head(3))
...
#
#      A partir de DTOBITO, crie as colunas:
#          ANO_OBITO  — ano do óbito (int)
#          MES_OBITO  — mês do óbito (int, 1 a 12)
#
#      Mostre:
#          - o óbito mais antigo e o mais recente na base (min e max de DTOBITO)
#          - a frequência de ANO_OBITO
# -----------------------------------------------------------------------------

print('\n1.b — Conversão de datas e criação de ANO_OBITO e MES_OBITO')
...
# -----------------------------------------------------------------------------
# 2.b) Agrupando dfINMET por UF, calcule em um único .agg() com dicionário:
#          - média de temperatura
#          - total (soma) de precipitação
#          - média de umidade relativa
#          - número de registros (count de temperatura)
#
#      Ordene pelo maior total de precipitação.
#      Mostre o resultado completo.
# -----------------------------------------------------------------------------

print('\n2.b — Estatísticas meteorológicas por UF')
...
#
#      Compare — por ESTACAO_CHUVA e UF juntos (groupby com lista) —
#      a média de temperatura e a média de precipitação.
#      Mostre os resultados ordenados por UF.
# -----------------------------------------------------------------------------

print('\n2.c — Temperatura e precipitação: estação chuvosa vs. seca por UF')
